...
// echo "index_cust_computers.php    Session variables are set. vEntity=$vEntity    , vCustomer=$vCustomer <br>   ";
?>
<!DOCTYPE html>
<html lang="en">
<head>
...
?>
    <div class="container">
            <div class="row">
                <h4>Gestion des Contacts/Site</h3>
            </div>
            <div class="row">
                ...
                <table class="table table-striped table-bordered">
				   ...
				   where is_deletecust=0 order by companynament, customer_name DESC';	
							foreach ($pdo->query($sql) as $row) {
                            echo '<tr>';
							echo '<td>'. $row['companynament'] . '</td>';

echo '<td>'.'<a href="contact_list.php?customer_id='.$row['idencust'].'&customer_name='.$row["customer_name"].'&entities_id='.$row['entities_id'].'&companynament='.$row['companynament'].'&first=0'.'">'. $row["customer_name"]. '</a>'. '</td>';	
                            echo ' ';			
