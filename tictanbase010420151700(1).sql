...
--
-- Client: localhost
-- Généré le: Mer 01 Avril 2015 à 17:00
...
--
-- Doublure de structure pour la vue `vtictan_customers_computerss`
--
CREATE TABLE IF NOT EXISTS `vtictan_customers_computerss` (
`id` int(11)
,`customers_id` int(11)
,`entities_id` int(11)
,`pcname` varchar(50)
...
--
-- Structure de la vue `vtictan_customers_computerss`
--
DROP TABLE IF EXISTS `vtictan_customers_computerss`;

CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vtictan_customers_computerss` AS select `a`.`id` AS `id`,`a`.`customers_id` AS `customers_id`,`a`.`entities_id` AS `entities_id`,`a`.`pcname` AS `pcname`,`a`.`processor` AS `processor`,`a`.`ram` AS `ram`,`a`.`serial` AS `serial`,`a`.`computermodels_id` AS `computermodels_id`,`a`.`computertypes_id` AS `computertypes_id`,`a`.`domain_name` AS `domain_name`,`a`.`session_admin_name` AS `session_admin_name`,`a`.`session_admin_pwd` AS `session_admin_pwd`,`a`.`session_user_name` AS `session_user_name`,`a`.`session_user_pwd` AS `session_user_pwd`,`a`.`teamv_logname` AS `teamv_logname`,`a`.`teamv_pwd` AS `teamv_pwd`,`a`.`adresseip` AS `adresseip`,`a`.`typeconnexion` AS `typeconnexion`,`a`.`comment` AS `comment`,`a`.`location` AS `location`,`a`.`author` AS `author`,`a`.`mod_date` AS `mod_date`,`a`.`is_deleted` AS `is_deleted`,`b`.`name` AS `modelname`,`e`.`companyname` AS `companynament`,`e`.`mobile` AS `mobile_ent`,`e`.`phone` AS `phone_ent`,`e`.`email` AS `email_ent`,`f`.`customer_name` AS `customer_name`,`f`.`mobile` AS `mobile_cust`,`f`.`phone` AS `phone_cust`,`f`.`email` AS `email_cust`,`g`.`name` AS `typename`,`g`.`is_deleted` AS `is_deletedtype` from ((((`tictan_customers_computers` `a` join `tictan_computermodels` `b`) join `tictan_entities` `e`) join `tictan_customers` `f`) join `tictan_computertypes` `g`) where ((`a`.`computermodels_id` = `b`.`id`) and (`a`.`computertypes_id` = `g`.`id`) and (`a`.`entities_id` = `e`.`id`) and (`a`.`customers_id` = `f`.`id`));
