...
?>
    ...
    <div class="container">
            ...
            <div class="row">
				...
				$sql = 'select ident, idencust, entities_id,companynament, customer_name, addresscust, 
				towncust, mobilecust, emailcust from vtictan_customers 
				where is_deletecust=0 order by customer_name DESC';	
							foreach ($pdo->query($sql) as $row) {
                            echo '<tr>';
							echo '<td>'. $row['companynament'] . '</td>';

echo '<td>'.'<a href="index_cust.php?customer_id='.$row['idencust'].'&customer_name='.$row["customer_name"].'&entities_id='.$row['entities_id'].'&companynament='.$row['companynament'].'&first=0'.'">'. $row["customer_name"]. '</a>'. '</td>';	
                            echo ' ';			
