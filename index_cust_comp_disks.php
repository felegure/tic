...
// echo "entities_id=$entities_id, customers_id:$customers_id, computers_id=$computers_id pcname=$pcname<br>";
 if ($_SESSION['profilAccess'] == 'A' || $_SESSION['profilAccess'] == 'S') 	
  echo ' <a href="create_cust_comp_disks.php?pcname='.$pcname.'&entities_id='.$entities_id.'&companyname='.$companyname.'&customers_id='.$customers_id.'&customer_name='.$customer_name.'&computers_id='.$computers_id.'" class="btn btn-success">Ajout Disque</a>';
 else
  echo ' <a href="create_cust_comp_disks.php?pcname='.$pcname.'&entities_id='.$entities_id.'&companyname='.$companyname.'&customers_id='.$customers_id.'&customer_name='.$customer_name.'&computers_id='.$computers_id.'"  onClick="ValidateON()"  class="btn btn-success">Ajout Disque</a>';
?>
 </p>
 ...
 <?php
  ...
  echo "<h5> Liste des disques du Client $companyname / $customer_name </h5>";
//  echo "customers_id=$customers_id <br>";
  $sql = 'select * from vtictan_cust_comp_disk where customers_id='; 
...
//  echo "sql = $sql <br>";
  ...
  echo '<a class="btn" href="read_cust_comp_disks.php?id='.$row['id'].'&pcname='.$pcname.'&entities_id='.$entities_id.'&companyname='.$companyname.'&customers_id='.$customers_id.'&customer_name='.$customer_name.'&computers_id='.$computers_id.'">Voir</a>';
  echo ' ';
  if ($_SESSION['profilAccess'] == 'A' || $_SESSION['profilAccess'] == 'S') {
  echo '<a class="btn btn-success" href="update_cust_comp_disks.php?id='.$row['id'].'&pcname='.$pcname.'&entities_id='.$entities_id.'&companyname='.$companyname.'&customers_id='.$customers_id.'&customer_name='.$customer_name.'&computers_id='.$computers_id.'&name='.$row['name'].'">Modifier</a>';  
  echo ' ';
  echo '<a class="btn btn-danger" href="delete_cust_comp_disks.php?id='.$row['id'].'&pcname='.$pcname.'&entities_id='.$entities_id.'&companyname='.$companyname.'&customers_id='.$customers_id.'&customer_name='.$customer_name.'&computers_id='.$computers_id.'">Supprimer</a>';
  }
  echo '</td>';
  echo '</tr>';
  } 
...
<table  width="900" height="30" border="0" align="center" cellpadding="1" cellspacing="1" bgcolor="#ECE9D8" frame="border">
 ...
 <a class="btn" href="disks_list.php?entities_id=<?php echo $entities_id; ?>&customers_id=<?php echo $customers_id; ?>&customer_name=<?php echo $customer_name;?> &type=disks">Retour</a>
	  </td>  
 </tr> 
  </table>	
 </div>			
