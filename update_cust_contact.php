<?php
    ...
    echo "dans post <br>";
        ...
        if ($valid) {
			...
			mobile =? ,fax = ?,email = ?,contacttypes_id = ?,comment = ?, address = ? ,postcode = ? , town = ?, 
			country = ?, author = ?, date_mod = ?, is_deleted = ? WHERE id = ?";
			
            $q = $pdo->prepare($sql);
            $q->execute(array( $name, $firstName, $phone, $phone2, $mobile, $fax, $email, $contacttypes_id, $comment,
			$address, $postcode, $town, $country, $author, $date_mod, $is_deleted, $id));
            Database::disconnect();
...
//	       Database::disconnect();	
	   $pdo = Database::connect();
        $pdo->setAttribute(PDO::ATTR_ERRMODE, PDO::ERRMODE_EXCEPTION);
		$sqlview = "SELECT * FROM vtictan_customers_contacts where customers_id = $customers_id 
		and entities_id = $entities_id and is_deleted = 0 ";
                ...
                <div class="span10 offset1">
                    ...
                    <form class="form-horizontal" action="update_contact.php?id=<?php echo $id?>" method="post">
					  <div class="control-group <?php echo !empty($entities_idError)?'error':'';?>">
						 ...
						 <div class="controls">
							 <input name="entities_id" type="text"  placeholder="entities_id" readonly="true" value="<?php echo !empty($companyname)?$companyname:'';?>">
                     ...
                     <label class="control-label">Prenom du contact</label>
                       ...
                       <label class="control-label">Type de contact</label>
						 <div class="controls">
						 <input name="contacttypes_id" type="text" placeholder="contacttypes_id" readonly="true" value="<?php echo !empty($contactypename)?$contactypename:'';?>">
                            <?php if (!empty($contactypenameError)): ?>
                                <span class="help-inline"><?php echo $contactypenameError;?></span>
                            <?php endif; ?>
