...
--
-- Client: localhost
-- Généré le: Jeu 30 Avril 2015 à 11:41
...
(15, 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', '', '', 11, 9, 4, 4, '0000-00-00 00:00:00', 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', 'pcme 9 mere2Clientmarche bio', NULL, '', 'pcme 9 mere2Clientmarche bio', '', '', 'pcme 9 mere2Clientmarche bio', 0, '0.0000', NULL, 'pcme 9 mere2Clientmarche bio', 'admin', '0000-00-00'),
...
CREATE TABLE IF NOT EXISTS `tictan_cust_comp_disk` (
  `id` int(11) NOT NULL AUTO_INCREMENT,
  `diskname` varchar(50) COLLATE utf8_unicode_ci DEFAULT NULL,
  `serial` varchar(50) COLLATE utf8_unicode_ci DEFAULT NULL,
  ...
  `date_mod` datetime DEFAULT NULL,
  `partitioname1` varchar(20) COLLATE utf8_unicode_ci DEFAULT NULL,
  `partitioname2` varchar(20) COLLATE utf8_unicode_ci DEFAULT NULL,
  `partitioname3` varchar(20) COLLATE utf8_unicode_ci DEFAULT NULL,
  `is_deleted` int(1) NOT NULL DEFAULT '0',
  `comment` text COLLATE utf8_unicode_ci,
  `author` varchar(30) COLLATE utf8_unicode_ci NOT NULL DEFAULT 'felicien',
  `mod_date` date NOT NULL,
...
INSERT INTO `tictan_cust_comp_disk` (`id`, `diskname`, `serial`, `supplier`, `customers_id`, `entities_id`, `computers_id`, `diskmodels_id`, `disktypes_id`, `date_mod`, `partitioname1`, `partitioname2`, `partitioname3`, `is_deleted`, `comment`, `author`, `mod_date`) VALUES
...
(5, 'Nom de disque 3', 'serie noire', 'fournisseur', 22, 22, NULL, 7, 7, '0000-00-00 00:00:00', 'partition 1', 'partition 2', '', 0, 'COçmme', 'admin', '0000-00-00'),
...
(31, 'BIDON 4444', '', '', '', 0, '', NULL, '', '', '', '', '', '', '', '', NULL, '', 1, 'admin', NULL);
...
,`diskname` varchar(50)
,`is_deleted` int(1)
,`supplierdisk` varchar(50)
,`customers_id` int(11)
,`entities_id` int(11)
,`serialdisk` varchar(50)
,`computers_id` int(11)
,`comment` text
,`diskmodels_id` int(11)
,`disktypes_id` int(11)
,`date_mod` datetime
,`partitioname1` varchar(20)
,`partitioname2` varchar(20)
,`partitioname3` varchar(20)
,`author` varchar(30)
,`pcname` varchar(50)
,`customer_name` varchar(255)
,`mobile_cust` varchar(255)
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vtictan_cust_comp_disk` AS select `a`.`id` AS `id`,`a`.`diskname` AS `diskname`,`a`.`is_deleted` AS `is_deleted`,`a`.`supplier` AS `supplierdisk`,`a`.`customers_id` AS `customers_id`,`a`.`entities_id` AS `entities_id`,`a`.`serial` AS `serialdisk`,`a`.`computers_id` AS `computers_id`,`a`.`comment` AS `comment`,`a`.`diskmodels_id` AS `diskmodels_id`,`a`.`disktypes_id` AS `disktypes_id`,`a`.`date_mod` AS `date_mod`,`a`.`partitioname1` AS `partitioname1`,`a`.`partitioname2` AS `partitioname2`,`a`.`partitioname3` AS `partitioname3`,`a`.`author` AS `author`,`b`.`pcname` AS `pcname`,`f`.`customer_name` AS `customer_name`,`f`.`mobile` AS `mobile_cust`,`f`.`phone` AS `phone_cust`,`f`.`email` AS `email_cust`,`c`.`name` AS `namemodel`,`b`.`is_deleted` AS `is_deleted_model`,`d`.`name` AS `nametype`,`c`.`is_deleted` AS `is_deleted_type`,`e`.`companyname` AS `companynament`,`e`.`mobile` AS `mobile_ent`,`e`.`phone` AS `phone_ent`,`e`.`email` AS `email_ent` from (((((`tictan_cust_comp_disk` `a` join `tictan_customers_computers` `b`) join `tictan_diskmodels` `c`) join `tictan_disktypes` `d`) join `tictan_entities` `e`) join `tictan_customers` `f`) where ((`a`.`diskmodels_id` = `c`.`id`) and (`a`.`disktypes_id` = `d`.`id`) and (`a`.`entities_id` = `e`.`id`) and (`a`.`customers_id` = `f`.`id`) and (`a`.`computers_id` = `b`.`id`));
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vtictan_cust_comp_virtuels` AS select `a`.`id` AS `id`,`a`.`drivename` AS `drivename`,`a`.`is_deleted` AS `is_deleted`,`a`.`customers_id` AS `customers_id`,`a`.`entities_id` AS `entities_id`,`a`.`computers_id` AS `computers_id`,`a`.`comment` AS `comment`,`a`.`date_mod` AS `date_mod`,`a`.`author` AS `author`,`b`.`pcname` AS `pcname`,`f`.`customer_name` AS `customer_name`,`f`.`mobile` AS `mobile_cust`,`f`.`phone` AS `phone_cust`,`f`.`email` AS `email_cust`,`e`.`companyname` AS `companynament`,`e`.`mobile` AS `mobile_ent`,`e`.`phone` AS `phone_ent`,`e`.`email` AS `email_ent` from (((`tictan_cust_comp_virdrive` `a` join `tictan_customers_computers` `b`) join `tictan_entities` `e`) join `tictan_customers` `f`) where ((`a`.`entities_id` = `e`.`id`) and (`a`.`customers_id` = `f`.`id`) and (`a`.`computers_id` = `b`.`id`));
